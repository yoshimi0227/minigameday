...
| 柱 | 使うもの | 役割 |
|---|---|---|
| 1. シナリオ立案 | `gameday-scenario` エージェント | シナリオを設計し、`scenarios/NN-<slug>.md` に保存できる形式で返す(実装はしない) |
| 2. 障害実装 | `fis-experiment` スキル | シナリオを `lib/fis-stack.ts` の FIS 実験テンプレートに実装。安全装置チェックリスト付き |
...
lib/            スタック・コンストラクト定義
  app-stack.ts        対象アプリ 3層 (ALB / Fargate / Aurora Serverless v2)
  fis-stack.ts        FIS 実験テンプレート
  observability-*.ts  Synthetics canary / CloudWatch
