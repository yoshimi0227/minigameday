...
- **爆発半径**: EC2 1 台のみ。該当タグの付いたインスタンスは環境全体で 1 台であることをタグ検索で事前確認する。現行 AppStack (Fargate) には EC2 インスタンスが存在せず波及しない
...
**現行 `lib/app-stack.ts` は既に Fargate desiredCount=2 であり、EC2 インスタンスは存在しない。本シナリオは学習用の出発点として、あえて SPOF 構成の別スタック (仮称 `GameDay-LegacyApp`) を運営が事前デプロイすることが前提。**
...
- データ層: 既存 AppStack の Aurora を共有するか専用に立てるかは実装判断。共有する場合、**AppStack の Fargate サービスをこの ALB に載せないこと** (載せると SPOF でなくなり障害が成立しない)
...
- **drift + 棚卸し + IaC 反映の 3 点セット** (rebuild 型の振り返りの核):
  ...
  3. IaC 反映ワーク (宿題) — 復旧構成を CDK 化する。種明かし: 答えはこのリポジトリの `lib/app-stack.ts` とほぼ同型。手で 60 分かけた構成が `cdk deploy` 一発である事実が IaC の説得材料になる
...
2. 運営側: 出発点スタック (GameDay-LegacyApp) を destroy、canary の向き先を AppStack の ALB に戻して再デプロイ、復旧材料 (ECR リポジトリ・クラスタ・実行ロール・ハンドアウト) の destroy
...
実装: `lib/legacy-app-stack.ts` (スタック名 `GameDay-Legacy`)。本流 3 スタックとは独立してデプロイ・破棄できる。
