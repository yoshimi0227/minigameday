...
`aws:rds:failover-db-cluster` — 対象クラスタを ARN 指定 (実装: 本体スタックの FaultInjection Construct、実験名 `gameday-failover-db`)。
...
---
実施記録 (2026-07-11 リハーサル): 仮説「503 が出る」は外れ。期待する振る舞いを実測に合わせて本ファイルで改訂済み。(旧 retrospectives/ レポートは廃止 — 詳細は git 履歴)
