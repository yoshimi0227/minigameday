...
- データ: `dashboard/public/data/gameday.json` **このファイルの編集がこのスキルの最頻作業**。
  ただしこれは**実行時状態 (git 管理外)** — 定義の正はコミット対象の `dashboard/gameday.seed.json` で、
  実体が無ければ dev サーバ / ビルドが seed から生成する。**恒久的な定義変更 (インジェクト・ヒント・
  systems・scoring) は seed を編集**し、ゲーム中の記録 (response / commsScore / feedback 等) は実体に書く。
  FIS 実験は seed では `experimentTemplateName` (Name タグ) で指し、ID は dev サーバが実行時解決する
...
カードは折りたたみ可能で、タブで対象システムを切り替える (タブは手動切替のみ)。
...
置かない (少人数・少インジェクト運用では選択肢がほぼ無くノイズになるため)。
...
- inject に `experimentTemplateName` (Name タグ。dev サーバが `experimentTemplateId` に解決) を
  記入しておくと、実験開始で「⏱ 実験進行中 (armed)」バナーが出る。
...
- **⟳ リトライ (周回リセット)** — 振り返りフィードバック (KPT `feedback[]` か `review`) が残ると出る
  ...
  子プロセス実行** (実験停止 → revert-drift → gameday-score ワイプ → gameday.json を seed から再生成)。数分かかり、
...
2. 依頼内容をスキーマに従って反映する:
   - **インジェクト追加** — `injects[]` に追加。`time` は実時刻、`status` は最初 `"pending"`。FIS 実験と対応する場合は `scenarioId` を scenarios/ の id と一致させ(スコア表・フィルタ・振り返りの突き合わせに使う)、実験は `experimentTemplateName` (Name タグ) で指す。**次回以降も使う定義なら seed (`dashboard/gameday.seed.json`) にも同じ追加を入れる** (実体だけの編集は周回リセットで消える)。
...
- 講評は **KPT 形式で `feedback[]` に一本化** (独立した講評セクションやレポートファイルは作らない)。AI 講評は `author: "AI 講評"` 付きで KPT ボードに人間のフィードバックと並ぶ。
- **AI 講評の自動生成 (dev 専用)**: KPT カード内の「AI 講評を KPT で生成」ボタン (ReviewControl。`import.meta.env.DEV` ガードで静的ビルドには出ない) → dev サーバの `POST /api/review` → まず `dashboard/drift-detector.ts` が **`cdk drift` (CDK CLI)** を子プロセスで実行 (全スタック対象、240 秒で打ち切り、生出力をそのまま材料化) し、`dashboard/review-generator.ts` が **Bedrock Converse API** で LLM を呼び、events タイムライン・採点・ヒント消費・**drift (宣言されていない手動変更 = 想定外の手作業の検知)** から Keep/Problem/Try を生成して `feedback[]` に書き込む (updateGamedayJson 経由)。drift はベストエフォート (未デプロイ・未認証は UNAVAILABLE として LLM に渡り、講評自体は止まらない)。宣言済み手動対応の drift には `cdk deploy --revert-drift` での IaC 還元を try として出させる。
  - 認証: `AWS_BEARER_TOKEN_BEDROCK` (Bedrock API キー) か既定の AWS 認証チェーン。モデル既定 `apac.amazon.nova-lite-v1:0` / リージョン既定 `ap-northeast-1` (`GAMEDAY_REVIEW_MODEL` / `GAMEDAY_BEDROCK_REGION` で上書き — Claude 等の別モデルを使うなら推論プロファイル ID を渡す)。
