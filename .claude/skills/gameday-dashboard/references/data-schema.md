...
## 定義 (seed) と実行時状態 (実体) の分離

リポジトリはクローンして各自の GameDay を回す配布物なので、ファイルは 2 層に分かれる:

- **`dashboard/gameday.seed.json` (コミット対象 = 定義の正)** — event / systems / injects の静的
  フィールド / scoring / rounds だけを持つ初期状態。**恒久的な定義変更 (インジェクト追加・
  ヒント修正・構成図更新など) はこちらを編集する**。アカウント固有の値は持たない —
  FIS 実験は `experimentTemplateId` (ID) ではなく `experimentTemplateName` (Name タグ) で指す。
  混入は `dashboard/src/seed.test.ts` が検知する。
- **`dashboard/public/data/gameday.json` (git 管理外 = 実行時状態)** — 実体。無ければ dev サーバ /
  ビルドが seed からコピー生成し、dev サーバが `experimentTemplateName` → ID を FIS API で解決して
  書き込む (30 秒間隔)。ゲーム中の記録 (response / notes / commsScore / feedback / scoreOverride) は
  こちらに書く。**`npm run reset` (周回リセット) は実体を `dashboard/data-archive/gameday-<stamp>.json`
  に退避してから seed で作り直す** — 実体への場当たりな定義編集はリセットで消える。
  過去回を画面から参照したい場合は `dashboard/public/data/archive/YYYY-MM-DD.json` にコピーする
  (data-archive = 自動退避、public/data/archive = 公開用の手動コピー。別系統)。
...
| フィールド | 型 | 誰が書くか | 説明 |
|---|---|---|---|
| `experimentTemplateName` | string | **運営 (seed に記入)** | FIS テンプレートの Name タグ (例 `gameday-scale-to-zero`)。アカウント非依存なので seed にはこちらを書く。dev サーバが 30 秒間隔で ID に解決して `experimentTemplateId` へ書き込む |
| `experimentTemplateId` | string | dev サーバ (Name タグから解決) / 運営 (手動指定も可) | FIS 実験テンプレート ID。イベントと inject の突き合わせキー。これが無いと自動記録されず、開始ボタンも出ない |
...
講評は全て KPT 形式でここに載る (独立した講評セクション・レポートファイルは無い)。
AI 講評の再生成 (ダッシュボードのボタン / gameday-retrospective スキル)
...
インジェクトを 1 件追加して自動記録の対象にする (**seed に追記** → 実体にも反映するか
`npm run reset` で反映。`experimentTemplateName` は FIS テンプレートの Name タグ):
...
```json
{
  ...
  "status": "pending",
  "maxScore": 100,
  "experimentTemplateName": "gameday-task-latency"
}
```
