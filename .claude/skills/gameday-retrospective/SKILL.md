...
**機械記録のタイムライン (`events[]`)** が最も客観的な素材。gameday.json の `events[]` に「FIS 実験開始 (running) / 終了」「canary アラーム ALARM (影響開始) / OK (復旧)」「検知宣言 (ack)」が時刻付きで自動記録されている (GameEvents → DynamoDB → dev サーバ同期)。inject には導出済みの `impactStartAt` / `ackAt` / `recoveredAt` / `detectionMinutes` / `recoveryMinutes` と自動採点の内訳も残る。**「いつ気づいたか」を記憶に頼らず語れる**のがこの記録の価値で、タイムライン表はまず events[] から起こし、人間のメモで補完する。

**データ収集は 1 コマンドで束ねられる**: `bash scripts/collect-retrospective.sh <experiment-id> [canary] [stack]` が FIS タイムライン / canary 成功率 / ALB・Target 5xx (顧客影響) / cdk drift / git diff を 1 つの Markdown に出す。これを入力に講評する。
...
7. **ダッシュボードへの掲載 (振り返りレビュー)**: gameday.json の `review` セクションに講評を書き込む (スキーマは gameday-dashboard スキルの references/data-schema.md)。書いた瞬間にダッシュボード末尾へ「振り返りレビュー」として表示される。
   - `overall`: 総評 (2〜4 文。ゲーム全体を通した傾向と次への一歩)
   - `injects[]`: インジェクトごとに `headline` (一言) + `commentary` (events のタイムライン・検知/復旧の実測・ヒント消費・drift の有無を織り込んだ講評) + `wentWell` / `toImprove`
   - `reportPath`: このレポート (`retrospectives/...md`) へのパス
   - 共有したい学びは従来どおり `feedback[]` (KPT) にも転記する。
   - **AI 自動生成という選択肢**: ダッシュボードの「AI 講評を生成」ボタン (dev サーバの `POST /api/review` → `dashboard/review-generator.ts` が Bedrock Converse API で LLM (既定 Amazon Nova Lite) を呼ぶ) でも `review` を生成できる。ただし (a) 生成結果に **`reportPath` は含まれない**ので、レポートを保存したら手で追記する。(b) `review-generator.ts` の SYSTEM_PROMPT は採点ルーブリック (検知40/対応40/伝達20 等) を再掲しているので、**ルーブリックを変えたらそちらも同期する**。環境変数・モデル既定は gameday-dashboard スキルの「振り返りとの連携」を参照。

### リセット (次のラウンド前 / 撤収前)

**一括: `npm run reset`** (`scripts/reset-gameday.ts`) — ①実行中の実験停止 → ②`cdk deploy --all --revert-drift` (ドリフトをコードの状態へ) → ③DynamoDB `gameday-score` の全消去 (**`FIRED#` 冪等キーが残ると 2 周目のエスカレーションが発火しない**) → ④gameday.json をインジェクト定義だけに初期化 (旧データは `dashboard/data-archive/` へ退避) → ⑤`cdk drift` で No drift 確認、まで自動で行う。`--dry-run` で事前確認できる。
