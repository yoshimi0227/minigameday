...
> **1 発目はダッシュボードからも開始できる**: 画面上部の「▶ GameDay 開始」ボタン (dev サーバのみ) が
> 最初の `experimentTemplateId` 付きインジェクトの実験を開始する (`POST /api/start`)。
> 2 発目以降は下の CLI かスコアエスカレーションで。
...
### フェーズ5: 振り返り(AI 講評 = KPT)

講評は **KPT (Keep / Problem / Try) 形式で gameday.json の `feedback[]` に載る** — ダッシュボードの
KPT ボードに人間のフィードバックと `author: "AI 講評"` 付きで並ぶ (独立したレポートファイルは作らない)。
ゲーム中に自動記録された `events[]` (実験開始 / ALARM / OK / 検知宣言) のタイムライン・採点・
ヒント消費が講評の一次資料になる。

**生成方法は 2 つ (出力先は同じ feedback[])**:

- **ダッシュボードのボタン (dev 専用)**: KPT カード内の「AI 講評を KPT で生成」→ dev サーバの
  `POST /api/review` → `dashboard/review-generator.ts` が **Amazon Bedrock (Converse API)** で LLM を
  呼ぶ (既定モデル `apac.amazon.nova-lite-v1:0` / 東京リージョン。`GAMEDAY_REVIEW_MODEL` /
  `GAMEDAY_BEDROCK_REGION` で上書き — Converse で呼べるモデルなら差し替え可)。認証は
  `AWS_BEARER_TOKEN_BEDROCK` か既定の AWS 認証チェーン。再生成は AI 講評分だけ入れ替える。
- **Claude Code (`gameday-retrospective` スキル)**: cdk drift / git diff / canary スクリーンショット
  まで踏み込んだ厚い講評が要るとき。3 視点 (canary / drift / メトリクス) の分析込み。
...
振り返りフィードバック (KPT か review) が残ると、ダッシュボード上部にも「⟳ リトライ (周回リセット)」
ボタンが出る (dev サーバのみ)。2 段階クリックで同じ `npm run reset` を実行し、進行状況を画面に表示する。
