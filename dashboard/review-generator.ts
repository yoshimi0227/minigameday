...
// (events タイムライン・検知/復旧分数・ヒント消費・採点) から KPT 形式の講評を作る。
...
/**
 * gameday.json の実測データから KPT 形式の講評 (feedback[] へ追記するエントリ群) を生成する。
 * author は AI_FEEDBACK_AUTHOR 固定 (再生成時の入れ替えキー)。失敗時は throw。
 */
export async function generateReview(data: GamedayData): Promise<Feedback[]> {
      ...
      messages: [
        {
          role: 'user',
          content: [
            { text: `今回の GameDay の記録データ (gameday.json) を KPT 形式で講評せよ。\n\n${JSON.stringify(material)}` },
          ],
        },
      ],
