...
test('ラウンドフィルタでそのラウンドのインジェクトだけに絞られる', async () => {
  const { container, getByLabelText } = await renderApp();
  fireEvent.change(getByLabelText('ラウンド'), { target: { value: '1' } });
  const round1 = data.injects.filter((i) => i.round === 1);
  // round1 の inject 行 + 見出し 1 行
  expect(container.querySelectorAll('tbody tr.round-header')).toHaveLength(1);
  expect(container.querySelectorAll('tbody tr')).toHaveLength(round1.length + 1);
});
...
test('シナリオフィルタが表・チャート・フィードバックを一括でスコープする', async () => {
  ...
  // scenario-01 は inject-1 (採点済み) と inject-3 (armed) の 2 行 + ラウンド見出し 1 行。
  // チャートは時間記録のある inject-1 のみ
